...
class app2vec_estimator(BaseEstimator, ClassifierMixin):
	...
	'''
	def __init__(self, training_data, sg = 1,size = 100,alpha=0.025, window = 5,iter = 1, min_count = 0,sample = 0.001,workers = 3,min_alpha = 0.0001,n_estimators = 1000,):
		self.size = size
		self.sg = sg
		self.window = window
	...
	def fit(self, X, y):
		...
		self.model = Word2Vec(self.training_data,sg = self.sg,size = self.size,window = self.window,seed = 0,min_count = self.min_count,iter = self.iter,compute_loss=True,workers = 5)
	...
	def fit_evaluate(self,X,y,app2vec_model):

		self.model = app2vec_model
